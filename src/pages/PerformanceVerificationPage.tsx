...
export function PerformanceVerificationPage() {
  useEffect(() => {
    document.title = 'Performance Verification | SnapTrader AI'
  }, [])
  ...
  return (
    <div className="bg-deep pt-[72px]">
          ...
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              <h1 className="mb-5 max-w-[640px] text-4xl font-black leading-[1.08] tracking-tight text-ink md:text-5xl lg:text-[3.4rem]">
                Performance Verification: Live Results You Can{' '}
                <span className="text-gradient-brand">Audit Yourself</span>
              </h1>
              <p className="mb-3 text-lg font-semibold leading-relaxed text-ink">
                Anyone can post a screenshot. We publish the ledger.
              </p>
              <p className="mb-8 max-w-[560px] text-lg leading-relaxed text-muted-dark">
                Every trade our AI agents place is recorded, timestamped and
                made public — the wins, the losses, and everything in between.
                No edits. No exceptions.
              </p>
            ...
            <Reveal delay={120}>
              <div className="relative">
                ...
                />
                <TerminalFrame
                  title="Snap Trader AI · Public Trade Ledger"
                  titleExtra={
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="font-mono text-[9px] text-ink-soft/60">
                        TIMESTAMPS ARE FINAL · NOTHING IS REWRITTEN AFTER THE FACT
                      </p>
                      <p className="font-mono text-[9px] font-bold text-ink-soft">
                        {LEDGER_ROWS.length} ROWS SHOWN
                      </p>
                    </div>
                  </div>
                </TerminalFrame>
        ...
        <div className="mx-auto max-w-container px-4 md:px-6">
          <Reveal>
            <SectionHeader
              ...
              description="Most trading platforms ask you to trust a number. We'd rather hand you the evidence."
            />
          </Reveal>
      ...
      <section className="bg-deep py-20 lg:py-28">
        <div className="mx-auto max-w-container px-4 md:px-6">
          <Reveal>
            <SectionHeader title="Verification in Four Layers" />
          </Reveal>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {FOUR_LAYERS.map((l, i) => (
              <Reveal key={l.title} delay={i * 80}>
                <div className="relative h-full overflow-hidden rounded-2xl border border-border bg-navy p-7 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:border-accent/30">
                  ...
                  <h2 className="mb-2 text-base font-bold text-ink">{l.title}</h2>
                  <p className="text-sm leading-relaxed text-muted-dark">{l.description}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1fr_1.3fr] lg:gap-16">
            <Reveal>
              <SectionHeader
                align="left"
                className="mb-0"
                title="How an Agent Earns Its Verified Badge"
                description="A badge isn't given at launch. It's earned across a full market cycle."
              />
              {/* Badge visual */}
              <div className="mt-8 flex items-center gap-5 rounded-2xl border border-border bg-deep p-6 shadow-card">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-relaxed text-muted-dark">
                    Agents still building their record trade openly under an
                    Unverified label. Nothing runs in the dark.
                  </p>
                </div>
              </div>
            </Reveal>
              ...
              <div className="overflow-hidden rounded-2xl border border-border bg-deep shadow-card">
                {BADGE_REQUIREMENTS.map((req, i) => (
                  ...
                  >
                    ...
                    <p className="text-sm leading-relaxed text-muted-dark md:text-base">{req}</p>
                  </div>
                ))}
      ...
      <section className="border-t border-border bg-deep py-20 lg:py-28">
        <div className="mx-auto max-w-container px-4 md:px-6">
          <Reveal>
            <SectionHeader title="The Numbers We Publish" description="Updated daily:" />
          </Reveal>
            ...
            {NUMBERS_PUBLISHED.map((n, i) => (
              <Reveal key={n} delay={i * 70}>
                <div className="relative h-full overflow-hidden rounded-2xl border border-border bg-navy p-6 shadow-card">
                  <div aria-hidden="true" className="absolute inset-x-0 top-0 h-[3px] gradient-brand opacity-70" />
                  <p className="text-sm font-semibold leading-relaxed text-muted-dark">{n}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-container px-4 md:px-6">
          <Reveal>
            <SectionHeader
              ...
              description="Trust is defined by what a platform refuses to do."
            />
          </Reveal>
            ...
            {WONT_FIND.map((item, i) => (
              <Reveal key={item} delay={i * 60}>
                <div className="flex items-center gap-4 rounded-xl border border-border bg-deep p-4 shadow-card">
                  ...
                  <p className="text-sm font-semibold leading-relaxed text-muted-dark">{item}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="border-t border-border bg-deep py-20 lg:py-28">
        <div className="mx-auto max-w-container px-4 md:px-6">
          <Reveal>
            <SectionHeader title="Verify It Yourself — Three Steps" />
          </Reveal>
          ...
          <Reveal>
            <p className="mx-auto mt-12 max-w-2xl text-center text-base font-semibold leading-relaxed text-ink md:text-lg">
              Two separate sources. Identical numbers. If they ever disagree,
              the independent record is the one that counts — and we&apos;ll say
              so publicly.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mx-auto max-w-2xl px-4 text-center md:px-6">
          <Reveal>
            <h2 className="mb-4 text-3xl font-extrabold text-ink md:text-4xl">
              See the record{' '}
              <span className="text-gradient-brand">for yourself.</span>
            </h2>
            <div className="mt-6">
              <Button to="/leaderboard" size="lg" className="group">
                View the Live Leaderboard
